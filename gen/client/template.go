...
type TemplateGenClient struct {
	...
	// main model
	ClientModel template.TemplateModel
	ClientFuncs []TemplateGenClientFunc
	// response and request models
	ParamModels []template.TemplateModel
}

type TemplateGenClientFunc struct {
	Name           string
	Comment        string
	BelongToStruct template.TemplateModel
	Path           string
	Method         string
	RequestModel   template.TemplateModel
	ResponseModel  template.TemplateModel
}
...
func (tgc *TemplateGenClient) parseDefinitionProperty(modleName string, proName string, pro GenClientDefinitionPropertie) (*template.TemplateModelField, error) {
	tp := ""
	switch pro.Type {
	...
	return &template.TemplateModelField{
...
func (tgc *TemplateGenClient) parseDefinitions(defs map[string]GenClientDefinition) error {
	paramModelsMap := map[string]template.TemplateModel{}
	for name, def := range defs {
		s := strings.Index(name, ".")
		if s != -1 {
			name = name[s+1:]
		}
		f := template.TemplateModel{
...
func (tgc *TemplateGenClient) parseFuncs(funcs []GenClientFunc) error {
	...
	for _, clientFunc := range funcs {
		...
		f.RequestModel = template.TemplateModel{
		...
		for _, parameter := range clientFunc.Parameters {
			...
			field := template.TemplateModelField{
		...
		for code, response := range clientFunc.Responses {
			...
			if response.Schema.Ref != "" {
				...
				if model, ok := paramModelsMap[name]; ok {
					tmpModel := template.TemplateModel{
...
func (tgc *TemplateGenClient) RegisteFuncMap() {
	...
	tgc.RegisteTemplateFunc(map[string]interface{}{
		"isModel": func(m template.TemplateModel) bool {
			if m.ModelName == "" {
				return false
			}
			return true
		},
		"isStruct": func(m template.TemplateModel) bool {
...
func (tgc *TemplateGenClient) ParseSwagger(url string) error {
	...
	if strings.Contains(url, "http://") {
		body, err = httptool.NewHttpRequest(url, nil).Get()
		if err != nil {
