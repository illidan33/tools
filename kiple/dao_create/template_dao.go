...
var templateMethodTxt = `package {{$.PackageName}}
...
var templateMethodMap = map[string]string{
	"FetchBy%s": `func (d *{{$.ModelName}}) {{$.FuncName}}({{$.ConditionStr}}) ({{var $.EntityName}} *{{$.EntityPackageName}}{{$.EntityName}},err error) {
		if err = d.db.Model({{var $.EntityName}}).Where("{{$.WhereStr}}", {{$.ConditionFieldStr}}).First({{var $.EntityName}}).Error; err != nil{
			return 
		}
		return 
	}`,
	"UpdateBy%sWithStruct": `func (d *{{$.ModelName}}) {{$.FuncName}}({{var $.EntityName}} {{$.EntityPackageName}}{{$.EntityName}}) (error) {
		if err := d.db.Model({{var $.EntityName}}).Where("{{$.WhereStr}}", {{$.FieldStr}}).Updates({{var $.EntityName}}).Error; err != nil{
			return err
		}
		return nil
	}`,
	"UpdateBy%sWithMap": `func (d *{{$.ModelName}}) {{$.FuncName}}({{$.ConditionStr}}, args map[string]interface{}) (error) {
		entt := {{$.EntityPackageName}}{{$.EntityName}}{}
		if err := d.db.Model(entt).Where("{{$.WhereStr}}", {{$.ConditionFieldStr}}).Updates(args).Error; err != nil{
			return err
		}
		return nil
	}`,
	"BatchFetchBy%s": `func (d *{{$.ModelName}}) {{$.FuncName}}({{$.ConditionStr}})({{var $.EntityName}}List []{{$.EntityPackageName}}{{$.EntityName}}, err error) {
		err = d.db.Where("{{$.WhereStr}}", {{$.ConditionFieldStr}}).Find(&{{var $.EntityName}}List).Error
		return 
	}`,
}
...
var templateMethodUniqMap = map[string]string{
	"Create": `func (d *{{$.ModelName}}) Create({{var $.EntityName}} {{$.EntityPackageName}}{{$.EntityName}}) (error) {
		if err := d.db.Create({{var $.EntityName}}).Error; err != nil{
			return err
		}
		return nil
	}`,
	"Delete": `func (d *{{$.ModelName}}) Delete({{var $.EntityName}} {{$.EntityPackageName}}{{$.EntityName}}) (error) {
		if err := d.db.Delete({{var $.EntityName}}).Error; err != nil {
			return err
		}
		return nil
	}`,
	"FetchList": `func (d *{{$.ModelName}}) FetchList(size int32, offset int32, args map[string]interface{})({{var $.EntityName}}List []{{$.EntityPackageName}}{{$.EntityName}}, count int32, err error) {
		if size == -1 {
			err = d.db.Where(args).Offset(offset).Find(&{{var $.EntityName}}List).Count(&count).Error
		} else {
			err = d.db.Where(args).Offset(offset).Limit(size).Find(&{{var $.EntityName}}List).Count(&count).Error
		}			
		return 
	}`,
}
...
func (tm *KipleTemplateDao) ParseKipleDstTree(file *dst.File) error {
	...
	tm.ModelName = tm.InterfaceName + implNameFlag
